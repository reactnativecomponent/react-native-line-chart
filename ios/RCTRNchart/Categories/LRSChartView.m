...
-(UIView *)scrollBgView1{
    if (!_scrollBgView1) {
        _scrollBgView1 = [[UIScrollView alloc]initWithFrame:CGRectMake(0, TopHight, self.chartScrollView.bounds.size.width-5, self.chartScrollView.bounds.size.height)];
...
-(void)setLineColor:(NSString *)lineColor{
    if (_lineColor != lineColor && lineColor) {
        _lineColor = lineColor;
    }
}
//图层颜色
-(void)setLayerColor:(NSString *)layerColor{
    if (_layerColor != layerColor && layerColor) {
        _layerColor = layerColor;
    }
}
//详情小方块字体颜色
-(void)setLabelTextColor:(NSString *)labelTextColor{
    if (_labelTextColor != labelTextColor && labelTextColor) {
...
-(void)addLeftBezierPoint{
    ...
    if (_leftDataArr.count != 0) {
        ...
        gradientLayer.colors = @[(__bridge id)[UIColor colorWithHexString:_layerColor andAlpha:_upAlpha].CGColor,(__bridge id)[UIColor colorWithHexString:_layerColor andAlpha:_downAlpha].CGColor];
        ...
        shapeLayer.strokeColor = [UIColor colorWithHexString:_lineColor andAlpha:1.0].CGColor;
...
-(void)addDataPointWith:(UIView *)view andArr:(NSArray *)leftData{
    ...
    if (arr.count != 0) {
        ...
        for (int i = 0; i<arr.count; i++) {
            ...
            detailLabel.textColor = [UIColor colorWithHexString:_labelTextColor];
            ...
            float sizeH = 0.0;
            if (height *(1 - tempHeight/valueSpace) - btnW/2 -15 < 0) {
                sizeH = 0.0;
            }else{
            ...
            [detailLabel setLayerWidth:0.6f layerColor:[UIColor colorWithHexString:_labelLayerColor] radius:2.0f];
            if (i == arr.count - 1) {
...
//            UILabel *leftLabel = [[UILabel alloc]initWithFrame:CGRectMake(Xmargin/2+i*Xmargin-10, 6.5*XlabelMargin, Xmargin*1.5, 20)];
            ...
            leftLabel.textAlignment = NSTextAlignmentCenter;
            //        leftLabel.transform = CGAffineTransformRotate(leftLabel.transform, -M_PI_4);
...
-(void)addLines1With:(UIView *)view{
    ...
    if (_isShow) {
        ...
        for (int i = 0;i< _leftDataArr.count ;i++ ) {
            UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(marginWidth*i, labelHeight - 5, 1, 5)];
            label.backgroundColor = [UIColor colorWithRed:241/255.0 green:241/255.0 blue:241/255.0 alpha:1];
            if (i != 0) {
                [view addSubview:label];
            }
